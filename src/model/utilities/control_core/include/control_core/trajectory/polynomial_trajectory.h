...
/**
 * @brief Template based switch between scalar and Eigen
 */
...
/**
 * @brief Template based switch between dynamic and static
 */
template<typename ValueType, int rows>
class ConditionalData
{
public:
  ValueType v;
  ConditionalData(int size)
  {
    // general case: Constant rows
    v = ValueType::Zero();
  }
};
template<typename ValueType>
class ConditionalData<ValueType, Eigen::Dynamic>
{
public:
  ValueType v;
  ConditionalData(int size)
  {
    // specialize on case: Dynamic rows
    v = ValueType::Zero(size);
  }
};
...
public:
  ...
  enum
  {
    Rows = Value::RowsAtCompileTime,
    Cols = Value::ColsAtCompileTime,
  };
...
protected:
  ...
  ConditionalData<Value, Rows> val_template_;
...
public:
  ...
  PolynomialTrajectory(
    ...
    val_template_(polynomials[0].size())
  {
  ...
  PolynomialTrajectory(
    ...
    polynomials_({polynomials}),
    val_template_(polynomials.size())
  {
    bool is_valid = Base::segments_.size()-1 == polynomials_.size();
    ROS_ASSERT_MSG(is_valid,
                  "segments size %ld must match polynomials size %ld",
  ...
  {
    ValueVec v_vec(t_vec.size(), val_template_.v);
   ...
   */
  ValueVec evaluateAll(const Scalar& t) const
  {
    ValueVec values;
    ...
    // evaluate the segment at time t
    Scalar tt = enforceTimeRange(t, idx);
    evaluateAllSpecialized(tt, idx, values);
    return values;
  }
  ...
  ValueMatrix evaluateAll(const ScalarVec& t_vec) const
  {
    ...
    for(size_t i = 0; i < t_vec.size(); ++i)
    {
      ...
      // evaluate the segment at time t
      Scalar tt = enforceTimeRange(t_vec[i], idx);
      evaluateAllSpecialized(tt, idx, values_vec[i]);
    }
    return values_vec;
  }
...
private:
  ...
  void evaluateAllSpecialized(const Scalar& tt, int idx, ValueModVec& output) const
  {
    std::vector<Scalar> values;

    // resize to hold d+1 matrices
    output.resize(3, val_template_.v);

    // for each dimension
    for(size_t i = 0; i < polynomials_[0].size(); ++i)
    {
      // compute the derivatives + the value
      std::vector<Scalar> values = polynomials_[idx][i].evaluateAll(tt);
  ...
  void evaluateAllSpecialized(const Scalar& tt, int idx, ScalarVec& output) const
  {
    // returns a vector of size d+1 with derivatives + value
    output = polynomials_[idx][0].evaluateAll(tt);
  }
