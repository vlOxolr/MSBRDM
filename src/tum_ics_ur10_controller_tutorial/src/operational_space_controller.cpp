...
namespace tum_ics_ur_robot_lli
{
  namespace RobotControllers
  {
    ...
    void OperationalSpaceControl::setQInit(const JointState &q_init)
    {
      q_init_ = q_init;
    }

    void OperationalSpaceControl::setQHome(const JointState &q_home)
    {
      q_home_ = q_home;
    }

    void OperationalSpaceControl::setQPark(const JointState &q_park)
    {
      q_park_ = q_park;
    }
    ...
    bool OperationalSpaceControl::init()
    {
      ...
      if (!g_traj.enabled)
      {
        ROS_WARN_STREAM("[OperationalSpaceControl] traj/enabled is FALSE. Controller will hold (no drawing).");
      }
      else
      {
        ROS_INFO_STREAM("[OperationalSpaceControl] traj/enabled=TRUE. "
                        << "use_move_to_start=" << (g_traj.use_move_to_start ? "true" : "false")
                        << " move_time=" << g_traj.move_time
                        << " move_tol=" << g_traj.move_tol
                        << " fixed_z=" << (g_traj.fixed_z ? "true" : "false")
                        << " z_paper=" << g_traj.z_paper
                        << " fixed_x=" << (g_traj.fixed_x ? "true" : "false")
                        << " x_paper=" << g_traj.x_paper
                        << " T_end=" << g_traj.T_end);
      }
    ...
    bool OperationalSpaceControl::stop()
    {
      return true;
    }
    ...
    Vector6d OperationalSpaceControl::update(const RobotTime &time, const JointState &state)
    {
      ...
      // Reference joint velocity qdot_r
      ...
      // Publish desired trajectory and task-space error (when enabled)
      if (g_traj.enabled)
      {
    ...
    Vector6d OperationalSpaceControl::computeQdotR(double t_sec, const cc::JointPosition &q, double dt)
    {
      ...
      {
        // During SAFE, ensure we are not moving/drawing
        g_traj.moving = false;
      ...
      if (g_traj.use_move_to_start && !g_traj.draw_started)
      {
        ...
        const bool timeout = (elapsed_move >= (g_traj.safe_time + g_traj.move_time + 0.5)); // simple timeout guard

        if (reached || timeout)
        {
          g_traj.moving = false;
          g_traj.draw_started = true;
          g_traj.t_draw0 = t_sec; // start drawing
          resetIntegrators();
          resetMarkerNewSegment();
          if (do_debug)
      ...
      // 3) DRAW: task-space trajectory tracking
      if (!g_traj.draw_started)
      {
        // If use_move_to_start == false, start drawing immediately (with proper draw time)
        g_traj.draw_started = true;
    ...
    {
      // Use EF consistently with the Jacobian (J_ef_0)
